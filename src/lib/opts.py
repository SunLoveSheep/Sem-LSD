...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('task', default='ctdet',
                                 help='ctdet | ddd | multi_pose | exdet | ctdet_line')
        self.parser.add_argument('--dataset', default='None',
                                 help='coco | kitti | coco_hp | pascal | semantic_line_kaist')
        self.parser.add_argument('--num_classes', default=None, type=int,
                                 help='num of classes for a dataset. Default None, use value from dataset script.')
        ...
        self.parser.add_argument('--save_path', default='./cache/debug/',
                                 help='path to save output images.')
        self.parser.add_argument('--save_img', type=int, default=1,
                                 help='=0 for not saving drawn images, 1 for saving such images')
        self.parser.add_argument('--max_gt_per_image', default=100, type=int,
                                 help='maximum number of ground truth kept for each image.')
        
        # ctdet line
        self.parser.add_argument('--direct_loss', default='cls', type=str,
                                 help='With wh_direct line encoding, select the loss type for direction.'
                                      'Default is cls, cross-entropy classification loss. '
                                      'Also support reg, regression loss (not very good)')
        self.parser.add_argument('--wh_loss_type', default='L1', type=str,
                                 help='Type of wh loss to use. Support:'
                                      'L1: default normal L1 reg loss for w-w_pred & h-h_pred'
                                                         )
        self.parser.add_argument('--loss_hm_magnitude', action='store_true',
                                 help="If to use magnitude heat map loss.")
        self.parser.add_argument('--loss_hm_magnitude_pos_only', action='store_true',
                                 help="If to use magnitude heat map loss with positive gt positions only.")
        self.parser.add_argument('--loss_hm_magnitude_neg_only', action='store_true',
                                 help="If to use magnitude heat map loss with negative gt positions only.")
        self.parser.add_argument('--hm_magnitude_weight', type=float, default=1.0,
                                 help='weight of magnitude hm loss.')
        ...
        self.parser.add_argument('--vis_thresh_gbuilding', type=float, default=0,
                                 help='visualization threshold for ground building.')
        ...
        self.parser.add_argument('--arch', default='resnet_18',
                                 help='model architecture. Currently tested'
                                      'resnet_18 | resdcn_18 | dla_34 | dla-bam33_34'
                                      'resnext-101_101 | resnext-dcn-101_101 | hourglass_104')
        ...
        self.parser.add_argument('--optimizer', type=str, default='Adam',
                                 help='Optimizer to be used. Supported:'
                                      'Adam | SGD')
        ...
        self.parser.add_argument('--weight_decay', type=float, default=0,
                                 help='weight_decay value, default=0')
        self.parser.add_argument('--momentum', type=float, default=0.9,
                                 help='momentum used for SGD optimizer. Default=0.9')
        ...
        self.parser.add_argument('--no_shuffle', action='store_true',
                                 help='Force training data loader not shuffling the input data for val test.')
        ...
        self.parser.add_argument('--no_color_aug', action='store_true',
                                 help='not use the color augmenation '
                                      'from CornerNet')
        self.parser.add_argument('--color_aug_var', type=float, default=0.4,
                                 help='-var ~ var for color augmentation functions')
        # multi_pose
        ...
        # ctdet_line
        self.parser.add_argument('--direct_weight', type=float, default=1.0,
                                 help='loss weight for line direction in ctdet line.')
    ...
    def parse(self, args=''):
        ...
        opt.data_dir = os.path.join(opt.root_dir, 'data') if opt.data_dir == "None" else opt.data_dir
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        print("in opt.py, opt.num_classes: ", opt.num_classes)
        ...
        if opt.task == 'exdet':
            ...
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
        elif opt.task == 'ctdet_line':
            opt.heads = {'hm': opt.num_classes,
                         'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
            if opt.direct_loss == 'cls':
                    opt.heads.update({'direct': 2})
            if opt.loss_hm_magnitude:
                    opt.heads.update({'grad_magnitude': 1})
    ...
    def init(self, args=''):
        default_dataset_info = {
                'ctdet': {'default_resolution': [512, 512], 'num_classes': 80,
                                    ...
                                    'dataset': 'coco'},
                'ctdet_line': {'default_resolution': [512, 512], 'num_classes': 10,
                                             'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
        ...
        # Need to update dataset information here if opt.dataset is set. Otherwise, heads will not be correct.
        dataset = Struct(default_dataset_info[opt.task])
        print("In opt.py, opt.dataset: ", opt.dataset)
        if opt.dataset == "None":
            print("opt dataset is not set! Use default dataset according to task.")
