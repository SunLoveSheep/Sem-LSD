...
def combine_bboxes(xs_in, ys_in, wh_in):
    return torch.cat([xs_in - torch.abs(wh_in[..., 0:1] / 2),
                      ys_in - torch.abs(wh_in[..., 1:2] / 2),
                      xs_in + torch.abs(wh_in[..., 0:1] / 2),
                      ys_in + torch.abs(wh_in[..., 1:2] / 2)], dim=2)
...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    ...
    #                     xs + wh[..., 0:1] / 2,
    #                     ys + wh[..., 1:2] / 2], dim=2)
    bboxes = combine_bboxes(xs, ys, wh)
    detections = torch.cat([bboxes, scores, clses], dim=2)
      
    return detections


def ctdet_line_decode(heat, wh, reg=None, directs=None, cat_spec_wh=False, K=100, direct_loss='reg'):
    batch, cat, height, width = heat.size()
    ...
    if cat_spec_wh:
        ...
        wh = wh.view(batch, K, 2)
    directs = _tranpose_and_gather_feat(directs, inds)
    if direct_loss == 'cls':
        directs = directs.view(batch, K, 2)
        directs = torch.argmax(directs, dim=2)
        directs = directs.unsqueeze(2).to(torch.float)
    else:
        directs = directs.view(batch, K, 1)
    clses = clses.view(batch, K, 1).float()
    scores = scores.view(batch, K, 1)
    # bboxes = torch.cat([xs - wh[..., 0:1] / 2,
    #                     ys - wh[..., 1:2] / 2,
    ...
    #                     ys + wh[..., 1:2] / 2], dim=2)
    bboxes = combine_bboxes(xs, ys, wh)
    detections = torch.cat([bboxes, scores, directs, clses], dim=2)
