...
class DeformConv(nn.Module):
    def __init__(self, chi, cho):
        ...
        self.conv_ver = nn.Conv2d(chi, cho, kernel_size=(3, 3), stride=1,
                                  padding=(3, 1), dilation=(3, 1))
        self.conv_hor = nn.Conv2d(chi, cho, kernel_size=(3, 3), stride=1,
                                  padding=(1, 3), dilation=(1, 3))

    def forward(self, x):
        # x = self.conv(x)
        x_dcn = self.conv(x)
        x_ver = self.conv_ver(x)
        x_hor = self.conv_hor(x)

        # Sum then activate:
        x_sum = x_dcn + x_ver + x_hor
        x_sum = self.actf(x_sum)
...
def get_pose_net(num_layers, heads, down_ratio=4, head_conv=256):
