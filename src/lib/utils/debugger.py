...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    if dataset == 'coco_hp':
      ...
      self.colors_hp = [(255, 0, 255), (255, 0, 0), (0, 0, 255), 
        ...
        (255, 0, 0), (0, 0, 255), (255, 0, 0), (0, 0, 255),
        (255, 0, 0), (0, 0, 255)]
    if dataset == 'semantic_line_kaist':
      self.names = semantic_line_kaist_name
  ...
  def add_bbox_line(self, cord, cat, conf=1, show_txt=True, img_id='default', direct=0):
    cord = np.array(cord, dtype=np.int32)
    # cat = (int(cat) + 1) % 80
    cat = int(cat)
    # print('cat', cat, self.names[cat])
    c = self.colors[cat][0][0].tolist()
    ...
    font = cv2.FONT_HERSHEY_SIMPLEX
    cat_size = cv2.getTextSize(txt, font, 0.5, 2)[0]
    if direct == 1:  # direction lt2rb
        x_left = min(cord[0], cord[2])
        y_left = min(cord[1], cord[3])
        x_right = max(cord[0], cord[2])
        y_right = max(cord[1], cord[3])
    else:  # direction lb2rt
        x_left = min(cord[0], cord[2])
        y_left = max(cord[1], cord[3])
        x_right = max(cord[0], cord[2])
        y_right = min(cord[1], cord[3])
    cv2.line(self.imgs[img_id], (x_left, y_left), (x_right, y_right), c, thickness=2)
    if show_txt:
        cv2.rectangle(self.imgs[img_id],
                      (cord[0], cord[1] - cat_size[1] - 2),
                      (cord[0] + cat_size[0], cord[1] - 2), c, -1)
        cv2.putText(self.imgs[img_id], txt, (cord[0], cord[1] - 2),
                    font, 0.5, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)

  def add_bbox_line_gt(self, cord, cat, conf=1, show_txt=True, img_id='default', direct=0):
    cord = np.array(cord, dtype=np.int32)
    # cat = (int(cat) + 1) % 80
    cat = int(cat)
    # print('cat', cat, self.names[cat])
  ...
  def add_semanticline_line(self, cord, cat, conf=1, show_txt=True, img_id='default'):
    cord = np.array(cord, dtype=np.int32)
    cat = int(cat)
    c = self.colors[cat][0][0].tolist()
    if self.theme == 'white':
    ...
    font = cv2.FONT_HERSHEY_SIMPLEX
    cat_size = cv2.getTextSize(txt, font, 0.5, 2)[0]
    cv2.line(self.imgs[img_id], (cord[0], cord[1]), (cord[2], cord[3]), c, thickness=4)
    if show_txt:
      cv2.rectangle(self.imgs[img_id],
                    (cord[0], cord[1] - cat_size[1] - 2),
                    (cord[0] + cat_size[0], cord[1] - 2), c, -1)
  ...
  def add_line(self, line, cat, conf=1, show_txt=False, img_id='default'):
    ...
    cat_size = cv2.getTextSize(txt, font, 0.5, 2)[0]
    cv2.line(
      self.imgs[img_id], (line[0], line[1]), (line[2], line[3]), c, 2)
    if show_txt:
      cv2.line(self.imgs[img_id],
                    (line[0], line[1] - cat_size[1] - 2),
                    (line[0] + cat_size[0], line[1] - 2), c, -1)
      cv2.putText(self.imgs[img_id], txt, (line[0], line[1] - 2),
                  font, 0.5, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
  ...
  def save_all_imgs(self, path='./cache/debug/', prefix='', genID=False, cnt=0,
                    img_name=None):
    if genID:
      try:
    ...
    for i, v in self.imgs.items():
      str_idx = '{0:05d}'.format(cnt)
      cnt = cnt + 1
      img_name_touse = path + '/{}-{}-{}.png'.format(i, prefix, str_idx) if img_name is None \
          else path + img_name.split('/')[-1]
      cv2.imwrite(img_name_touse, v)
