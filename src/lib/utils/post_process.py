...
def ctdetline_post_process(dets, c, s, h, w, num_classes):
  # dets: batch x max_dets x dim
  # return 1-based class det dict
  ret = []
  for i in range(dets.shape[0]):
    ...
    for j in range(num_classes):
      ...
      top_preds[j + 1] = np.concatenate([
        ...
        dets[i, inds, 5:6].astype(np.int)], axis=1).tolist()
    ret.append(top_preds)
  return ret


def ctdet_mot_post_process(dets, c, s, h, w, num_classes):
  # dets: batch x max_dets x dim
  # return 1-based class det dict
  ret = []
  for i in range(dets.shape[0]):
...
def ctdetline_anglen_post_process(dets, c, s, h, w, num_classes):
  # dets: batch x max_dets x dim
  # return 1-based class det dict
  ret = []
  for i in range(dets.shape[0]):
    ...
    for j in range(num_classes):
      ...
      top_preds[j + 1] = np.concatenate([
        dets[i, inds, :4].astype(np.float32),
        dets[i, inds, 4:5].astype(np.float32),
        dets[i, inds, 5:6].astype(np.float32),
        dets[i, inds, 6:7].astype(np.float32)], axis=1).tolist()
    ret.append(top_preds)
  return ret

def linereg_post_process(dets, c, s, h, w, num_classes):
  # dets: batch x max_dets x dim
  # return 1-based class det dict
  ret = []
  for i in range(dets.shape[0]):
