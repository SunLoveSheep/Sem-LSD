...
class CTDetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if input_h == input_w == 300 and self.opt.down_ratio == 8:
            input_h += 4
            input_w += 4
        ...
        for k in range(num_objs):
            ...
            try:
                ...
                bbox = self._semline_cord_to_box(ann['cord'])
            try:
                if self.opt.dataset == 'obj_kaist':
                    cls_id = int(self.cat_ids[ann['category_id'] - 14])
                else:
                    cls_id = int(self.cat_ids[ann['category_id']])
            except KeyError:
                print("Found KeyError, skip image {}...".format(img_path))
                continue

            # Make sure (bbox[0], bbox[1]) are min(x) and min(y), similar for bbox[2] and bbox[3].
