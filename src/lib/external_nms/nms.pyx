...
def nms_line(np.ndarray[np.float32_t, ndim=2] dets, np.float thresh):
    cdef np.ndarray[np.float32_t, ndim=1] x1 = dets[:, 0]
    cdef np.ndarray[np.float32_t, ndim=1] y1 = dets[:, 1]
    cdef np.ndarray[np.float32_t, ndim=1] x2 = dets[:, 2]
    cdef np.ndarray[np.float32_t, ndim=1] y2 = dets[:, 3]
    cdef np.ndarray[np.float32_t, ndim=1] scores = dets[:, 4]
    cdef np.ndarray[np.float32_t, ndim=1] directions = dets[:, 5]

    cdef np.ndarray[np.float32_t, ndim=1] areas = np.abs((x2 - x1 + 1) * (y2 - y1 + 1))
    cdef np.ndarray[np.int_t, ndim=1] order = scores.argsort()[::-1]
    ...
    for _i in range(ndets):
        ...
        for _j in range(_i + 1, ndets):
            ...
            h = max(0.0, yy2 - yy1 + 1)
            inter = w * h
            if iarea + areas[j] - inter == 0.0:
                print(iarea, areas[j], inter)
            if inter == 0.0:
                ovr = 0.0
            else:
                ovr = inter / (iarea + areas[j] - inter)
            if ovr >= thresh and directions[i] == directions[j]:
                suppressed[j] = 1
